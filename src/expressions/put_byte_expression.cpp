...
PutByteExpression::PutByteExpression(const yy::location location,
...
TypedResult<TypeSpecifier> PutByteExpression::GetTypeSpecifier(
...
const_shared_ptr<Result> PutByteExpression::Evaluate(
	...
	if (ErrorList::IsTerminator(errors)) {
		...
		auto value_expression = GetArgumentListRef()->GetNext()->GetData();
		auto value_evaluation = value_expression->Evaluate(context, closure);

		errors = value_evaluation->GetErrors();
		if (ErrorList::IsTerminator(errors)) {
...
TypedResult<string> PutByteExpression::ToString(
		...
		if (ErrorList::IsTerminator(errors)) {
			...
			if (ErrorList::IsTerminator(errors)) {
				...
				if (arg_count == 0) {
					...
					auto specifier = PrimitiveTypeSpecifier::GetByte();
					auto assignability =
							argument_type_specifier->AnalyzeAssignmentTo(
									specifier, type_table);
					if (assignability == AnalysisResult::AMBIGUOUS) {
